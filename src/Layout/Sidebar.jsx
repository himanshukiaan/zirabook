  ...
  const renderDropdownSection = (title, mainItem, subItems) => {
    ...
    return (
      <div className="mb-3" key={mainItem.to}>
        ...
        >
          <div
            className={`nav-link d-flex align-items-center sidebar-link px-3 py-2 ${activePath === mainItem.to || subItems.some((item) => item.to === activePath) ? "active-link" : ""}`}
            style={linkStyle}
          >
          ...
          <div className="submenu ps-4">
            {subItems.map((item) => (
              <div className="nav-item" key={item.to}>
                ...
                >
                  <i className={`me-2 ${item.icon}`} style={{ ...iconStyle, fontSize: "14px" }}></i>
    ...
    const menuItems = {
      ...
      Company: [
        ...
        renderFlatSection("Accounts", [
          ...
          { to: "/company/transaction", icon: "fas fa-money-check-alt", label: "All Transaction" },
        ]),
