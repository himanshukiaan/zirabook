...
const InventoryDetails = () => {
  ...
  // Sample data with purchase, sales, and return records
  const [data] = useState([
    {
      date: "01/03/2025",
      vchType: "Purchase",
  ...
  useEffect(() => {
    if (location.state && location.state.item) {
      ...
      // If no item data, navigate back to inventory list
  ...
  const handleSendEmail = () => {
    ...
    const body = "Yahan par item ka data aayega...";
  ...
  // Separate purchase, sales, and return history
  ...
  const totalReturns = returnHistory.reduce((sum, item) => {
    // For returns, we consider both positive inwards (sales return) and negative inwards (purchase return)
  ...
  return (
    <div className="">
      <div className="bg-white h-auto p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <Button onClick={handleSendEmail} className="btn-sm">Send </Button>
          </div>
          <Button 
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((row, index) => (
                    <tr key={index} className="hover:bg-gray-100">
                      ...
                      <td className="border p-2">{row.vchNo}</td>
